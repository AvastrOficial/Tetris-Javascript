class Game {
    ...
    initSounds() {
        this.sounds.background = Utils.loadSound("assets/s.mp3", true);
...
document.querySelector("#reset").addEventListener("click", () => {
 ...
 // Example JavaScript code to update the score and apply animation
        let score = 0;
        const puntajeElement = document.getElementById('puntaje');

        function updateScore() {
            score += 10;
            puntajeElement.textContent = score;
            puntajeElement.classList.add('updated');

            // Remove the 'updated' class after a delay to reset the color
            setTimeout(() => {
                puntajeElement.classList.remove('updated');
            }, 500);
        }

        // Call the updateScore function as needed
        updateScore();

 
   // Crea el objeto de audio
    const audio = new Audio('https://files.catbox.moe/tdy1qf.m4a');
   ...
   function toggleMusic() {
        ...
        if (audio.paused) {
            ...
            // Muestra los botones de volumen cuando la música está reproduciéndose
            decreaseVolumeBtn.style.display = 'inline-block';
            increaseVolumeBtn.style.display = 'inline-block';
        } else {
            audio.pause();
            musicIcon.className = 'fas fa-play';
            // Oculta los botones de volumen cuando la música está pausada
            decreaseVolumeBtn.style.display = 'none';
            increaseVolumeBtn.style.display = 'none';
        }
    }
 ...
 function changeColor() {
        // Cambia el color manualmente al hacer clic
        const header = document.querySelector('h1.auto-change');
